...
def tokenize(inpath:Path) -> list[Token]:
    ...
    while len(src.value) > 0:
        ...
        if eat_keyword(src, tokens): continue
        if eat_symbol(src, tokens): continue
        if eat_integer_constant(src, tokens): continue
        if eat_string_constant(src, tokens): continue
        if eat_identifier(src, tokens): continue
...
def eat_integer_constant(src:Ref[str], tokens:list[Token]) -> bool:
...
def eat_string_constant(src:Ref[str], tokens:list[Token]) -> bool:
    if src.value[0] == '"':
        i = 1
